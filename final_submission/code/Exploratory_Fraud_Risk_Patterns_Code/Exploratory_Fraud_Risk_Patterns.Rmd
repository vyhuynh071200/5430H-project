...
used only for compact code submission and the grouped failed-attempt
distribution. Because it is deliberately balanced, it cannot estimate
...
The full dataset is used for counts and fraud-rate estimates. The small balanced
sample is used only where equal group sizes make distributional shapes easier
to compare and where a runnable submission sample is required. Every
sample-based caption explicitly warns that its shares do not estimate
population prevalence. This separation prevents the 50% fraud share of the
balanced sample from being confused with the 5.53% fraud rate in the full data.
...
plot. The city comparison includes approximate 95% intervals to communicate
sampling uncertainty. Because the city figure uses a balanced sample rather
than a simple random population sample, those intervals are descriptive and
should not be interpreted as population estimates.

## Visualization Strategy

Bar charts compare rates across binary and categorical variables. Within-group
proportions compare failed-attempt distributions without being dominated by
class size. A heatmap combines hour of day and merchant category. For grouped
full-data comparisons, total transactions, fraud counts, and fraud rates are
calculated directly from `df`. Findings are described as visual associations,
not causal effects.
...
frequency and within-type composition; grouped bars preserve the discrete
nature of failed attempts; violin and box plots show density, median, and
...
```{r fig-failed-attempts, fig.pos="!htbp", fig.width=3.45, fig.height=2.8, out.width="\\columnwidth", fig.cap="Discrete failed-attempt distributions by sampled fraud group. Proportions are calculated separately within each 1,000-observation group."}
failed_distribution <- sample_df |>
  dplyr::filter(!is.na(failed_attempts), !is.na(fraud_label)) |>
  dplyr::count(fraud_label, failed_attempts, name = "transactions") |>
  dplyr::group_by(fraud_label) |>
  dplyr::mutate(proportion = transactions / sum(transactions)) |>
  dplyr::ungroup()

ggplot(
  failed_distribution,
  aes(factor(failed_attempts), proportion, fill = fraud_label)
) +
  geom_col(position = position_dodge(width = 0.78), width = 0.70) +
  geom_text(
    aes(
      label = paste0("n=", transactions, "\n",
  ...
  scale_fill_manual(values = fraud_colours, name = NULL) +
  scale_y_continuous(
    labels = scales::label_percent(accuracy = 1),
    expand = expansion(mult = c(0, 0.14))
  ) +
  labs(x = "Failed attempts", y = "Within-group proportion")
```

The balanced-sample comparison shows that fraud observations contain a larger
share of transactions with two or more failed attempts. This distribution is
consistent with the full-data result that fraud rates rise sharply beyond one
attempt. Because the plot uses within-group sample proportions, it describes
group composition rather than population prevalence. In the sample, 81.0% of
non-fraud observations have zero failed attempts, compared with 64.6% of fraud
observations. Conversely, two attempts account for 13.6% of fraud observations
but only 4.6% of non-fraud observations. Similar, smaller gaps continue from
three through five attempts, producing the clearest ordered behavioural pattern
in the analysis.

## Account Balance and Geographic Variation

```{r fig-account-balance, fig.pos="!htbp", fig.width=3.45, fig.height=2.75, out.width="\\columnwidth", fig.cap="Account-balance distributions by sampled fraud group. Violin width shows density and the inset boxes show the median and interquartile range."}
...
```{r fig-city-sample, fig.pos="!htbp", fig.width=3.45, fig.height=4.0, out.width="\\columnwidth", fig.cap="Sample fraud share by city with approximate 95% confidence intervals. The balanced sample is descriptive and does not estimate city-level population prevalence."}
city_sample_rates <- sample_df |>
  dplyr::filter(!is.na(city), !is.na(is_fraud)) |>
  dplyr::group_by(city) |>
  dplyr::summarise(
    n = dplyr::n(),
    fraud_share = mean(is_fraud == 1L),
    se = sqrt(fraud_share * (1 - fraud_share) / n),
    lower = pmax(0, fraud_share - 1.96 * se),
    upper = pmin(1, fraud_share + 1.96 * se),
    .groups = "drop"
  ) |>
  dplyr::filter(n >= 20) |>
  dplyr::arrange(fraud_share) |>
  dplyr::mutate(city = forcats::fct_reorder(city, fraud_share))

ggplot(city_sample_rates, aes(fraud_share, city)) +
  geom_vline(
    xintercept = mean(sample_df$is_fraud == 1L),
    colour = "#999999", linetype = "dotted", linewidth = 0.45
  ) +
  geom_errorbarh(aes(xmin = lower, xmax = upper), height = 0, colour = "#555555") +
  geom_point(colour = "#E45756", size = 1.7) +
  geom_text(
    aes(x = upper, label = paste0(" n=", n)),
    hjust = -0.08, size = 1.9, colour = "#5E6E86"
  ) +
  scale_x_continuous(
    labels = scales::label_percent(accuracy = 1),
    expand = expansion(mult = c(0.03, 0.20))
  ) +
  labs(x = "Fraud share in balanced sample", y = "City") +
  theme(axis.text.y = element_text(size = 6.5))
```

City-level sample shares vary, but the uncertainty intervals overlap widely.
The city comparison is therefore treated as a weak descriptive pattern rather
than a basis for geographic risk claims. The ordering ranges from higher sample
shares in Osaka and Guadalajara to lower shares in New York and Berlin, yet
most cities contain roughly 80--115 sample observations. The associated
uncertainty and synthetic nature of the locations make substantive geographic
...
type, distance from home, and city-level variation show weaker separation or
...
weekend status, account balance, and city provide weaker separation. Including
weak results is important because an EDA should narrow the candidate set rather
than merely collect visually interesting charts.
...
may hide repeated activity by the same account, and confidence intervals shown
for the balanced city sample do not correct for its case-control sampling
design. Finally, no classifier, train-test split, or cost-sensitive evaluation
is included because the proposal identified modeling only as an optional
extension if time permitted, while the primary deliverable remained EDA and
visualization.
...
Weekend status, account balance, and city-level variation are weaker or more
cautionary findings.
